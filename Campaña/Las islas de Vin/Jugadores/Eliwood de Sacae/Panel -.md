...
// 2. Buscar todos los objetos equipados en su directorio
const objetos = dv.pages()
    .where(p => p["Tipo-Nota"] === "Objeto" && p.Equipado === true && p.file.folder.includes(folderActual));

if (objetos.length > 0) {
    // --- ⚔️ SECCIÓN DE ARMAS ---
    const armas = objetos.where(o => o.Categoria === "Arma");
    if (armas.length > 0) {
        dv.header(4, "⚔️ Armas y Ataques Equipados");
        dv.table(
            ["Nombre", "🎯 Tipo", "🎲 Daño", "💪 Atrib.", "➕ Atk", "💥 Daño"],
            armas.map(a => [a.file.link, a.Tipo, a.DadoDano, a.AtributoAsociado, a.BonoAtaque, a.BonoDano])
        );
    }

    // --- 🛡️ SECCIÓN DE ARMADURAS ---
    const armaduras = objetos.where(o => o.Categoria === "Armadura");
    if (armaduras.length > 0) {
        dv.header(4, "🛡️ Armaduras y Defensas");
        dv.table(
            ["Nombre", "🛡️ AC Base", "🤸 Suma Dex", "🚫 Max Dex", "✨ Bono"],
            armaduras.map(ar => [ar.file.link, ar["AC-Base"], ar["Suma-Dex"] ? "Sí" : "No", ar["Max-Dex"], ar.BonoAC])
        );
    }

    // --- 🧪 SECCIÓN DE CONSUMIBLES / OTROS ---
    const otros = objetos.where(o => o.Categoria !== "Arma" && o.Categoria !== "Armadura");
    if (otros.length > 0) {
        dv.header(4, "📦 Consumibles y Otros Objetos Activos");
        dv.table(
            ["Nombre", "📂 Categoría", "📦 Cantidad"],
            otros.map(ot => [ot.file.link, ot.Categoria || "General", ot.Cantidad || 1])
        );
    }
} else {
